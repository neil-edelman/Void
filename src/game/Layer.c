...
/** Layers have objects that are ordered according to x and y.
...
struct Layer {
	...
	unsigned n;
	void *stuff;
	size_t x, y; /* offset (float) */
	size_t prev_x, next_x, prev_y, next_y; /* offset (*stuff) */
	void *first_x, *first_y, *iterator; /* (*stuff) */
	void *window_first_x, *window_first_y; /* (*stuff) */
	unsigned x_min, x_max, y_min, y_max; /* window */
	void *iterator; /* (*stuff) */
...
struct Layer *Layer(const void *const stuff, const size_t size, const size_t x,
					const size_t y, const size_t prev_x, const size_t next_x,
					const size_t prev_y, const size_t next_y,
					int half_max_size) {
	struct Layer *layer;

	if(!stuff || !size || x >= size || y >= size || prev_x >= size
	   || next_x >= size || prev_y >= size || next_y >= size
	   || half_max_size <= 0) {
	...
	layer->prev_y     = prev_y;
	layer->next_y     = next_y;
	layer->first_x = layer->first_y = 0;
	layer->window_first_x = layer->window_first_y = 0;
	layer->iterator   = 0;
...
int SpriteIterateAll(void *const stuff_ptr) {
...
void SpriteResetIterator(struct Layer *const layer) {
	if(!layer) return;
	layer->iterator = 0;
}
...
int LayerIterate(const struct Layer *const layer, void *const stuff_ptr) {
	...
	if(!layer->iterator) {
		...
		x_min_exp = layer->x_min - layer->half_max_size;
		/*fprintf(stderr, "window(%d:%d,%d,%d)\n", x_min, x_max, y_min, y_max);*/
		/* no sprite anywhere? */
		if(!layer->first_x_window && !(layer->first_x_window = layer->first_x)) return 0;
		/* place the first_x_window on the first x in the window;
		 fixme: this is messy and I just wrote it up ad-hoc */
		x = *(float *)((char *)layer->first_x_window + layer->x);
		if(x < x_min_exp) {
			do {
				if(!(next_x = *(void **)((char *)layer->first_x_window + layer->next_x))) return 0;
				layer->first_x_window = next_x;
				x = *(float *)((char *)layer->first_x_window + layer->x);
			} while(x < layer->x_min - layer->half_max_size);
		} else {
			while((feeler = *(void **)((char *)layer->first_x_window + layer->prev_x)
				   && (x_min_exp <= *(float *)((char *)feeler + layer->x))) {
				layer->first_x_window = feeler;
			}
		}
		...
		/* select the first y; fixme: should probably go the other way around,
		 ...
		 to mark more? */
		layer->iterator = layer->first_y_window;
		layer->is_listing = -1;
	}
