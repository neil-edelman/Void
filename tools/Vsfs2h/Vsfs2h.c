...
int main(int argc, char **argv) {
	...
	"\t\tDebug(\"auto_%s_shader_init: compiled vertex shader, Sdr%%u.\\n\", vs);\n"
	"", name, name);
	...
	"\t\tDebug(\"auto_%s_shader_init: compiled fragment shader, Sdr%%u.\\n\", fs);\n"
	...
	"\t\tDebug(\"auto_%s_shader_init: linked shader programme, Sdr%%u.\\n\", shader);\n"
	"\t\tglValidateProgram(shader);\n"
	"\t\tglGetProgramiv(shader, GL_VALIDATE_STATUS, &status);\n"
	"\t\tif(!status) { e = S_VALIDATE; break; }\n"
	"\t\tDebug(\"auto_%s_shader_init: validated shader, Sdr%%u.\\n\", shader);\n"
	"\t} while(0);\n"
	"\tif(e) {\n"
	"\t\tGLchar info[1024];\n"
	...
	"\t\tDebug(\"auto_%s_shader_init: failed %%s;\\n%%s\", during[e], info);\n"
	"\t}\n"
	"\tif(fs) {\n"
	"\t\tglDeleteShader(fs);\n"
	"\t\tif(shader) glDetachShader(shader, fs);\n"
	"\t\tDebug(\"auto_%s_shader_init: erasing fragment shader, Sdr%%u.\\n\", fs);\n"
	"\t}\n"
	"\tif(vs) {\n"
	"\t\tglDeleteShader(vs);\n"
	"", name, name);
	printf("\t\tif(shader) glDetachShader(shader, vs);\n"
	"\t\tDebug(\"auto_%s_shader_init: erasing vertex shader, Sdr%%u.\\n\", vs);\n"
	"\t}\n"
	"\tif(e) {\n"
	"\t\tif(shader) {\n"
	"\t\t\tglDeleteProgram(shader);\n"
	"\t\t\tDebug(\"auto_%s_shader_init: erased shader, Sdr%%u.\\n\", shader);\n"
	"\t\t}\n"
	"\t\treturn 0;\n"
	"\t}\n"
	"\tglUseProgram(shader);\n"
	...
	"\tDebug(\"~auto_%s_shader: erase shader, Sdr%%u.\\n\",\n"
